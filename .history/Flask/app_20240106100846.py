...
def predict_image():
    ...
    image_file = request.files['image']
    path = '../Client/src/assets/input'
    image_file.save(path + ".tif")
    shape = builder.insert_image(path+'.tif')
    shape.image_data.save(path + ".png")

    save_path = "../Client/src/assets/results.tif"

    path = '../Client/src/assets/results'
    saved_image_path = predict_and_return_tif(image_file, save_path)
    # Use Aspose.Words to directly save a modified TIFF image
    doc = aw.Document(save_path)  # Load the saved TIFF
    builder = aw.DocumentBuilder(doc)

    # ... (perform necessary image modifications using Aspose.Words)

    # Directly save the modified TIFF using Aspose.Words
    doc.save(save_path)
