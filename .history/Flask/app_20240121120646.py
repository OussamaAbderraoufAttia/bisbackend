...
def predict_and_return_png_paths(image_data, save_path_input_png, save_path_results_png):
    ...
    img_input = Image.fromarray(image)  # Create PIL Image from original image
    img_input.save(save_path_input_png, format="PNG")

    # Save prediction as PNG using Pillow (ensure binary_prediction is in RGB format)
    if len(binary_prediction.shape) == 2:  # Grayscale prediction
        binary_prediction = cv2.cvtColor(binary_prediction, cv2.COLOR_GRAY2RGB)  # Convert to RGB
    img_results = Image.fromarray(binary_prediction)
...
def index():
    if request.method == 'POST':
        ...
        save_path = "../Client/src/assets/"

        input_png_path = "../Client/src/assets/input.png"  
        results_png_path = "../Client/src/assets/results.png"  
        input_png_path, results_png_path = predict_and_return_png_paths(image_file, input_png_path, results_png_path)


        return jsonify({'message': 'Prediction completed and saved as TIFF image', 'image_path': save_path})
