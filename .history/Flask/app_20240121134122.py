...
def predict_and_return_png_paths(image_data, save_path_input_png, save_path_results_png):
    try:
        ...
        # Reshape and convert data type for Pillow compatibility
        binary_prediction = np.squeeze(binary_prediction, axis=2).astype(np.uint8)
        ...
        try:
            # Convert to standard NumPy array before saving
            img_results = Image.fromarray(np.array(binary_prediction), mode="RGB")
