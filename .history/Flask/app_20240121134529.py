...
def predict_and_return_png_paths(image_data, save_path_input_png, save_path_results_png):
    try:
        ...
        print("binary_prediction shape before conversion:", binary_prediction.shape)

        # Remove the singleton channel dimension
        binary_prediction = binary_prediction.squeeze(axis=2)

        # Enhanced error handling for saving
        try:
            # Convert to standard NumPy array and ensure it's in the correct data type
            img_results = Image.fromarray(binary_prediction.astype(np.uint8))
        ...
        print("binary_prediction shape after conversion:", binary_prediction.shape)
