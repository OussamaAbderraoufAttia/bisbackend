...
def predict_image():
    ...
    image_file = request.files['image']
    save_path = "path_to_save_result.tif"
    color_threshold = 0.00001  # You may adjust this threshold value
    result = detect_tumor_color(save_path, color_threshold)
    saved_image_path = predict_and_return_tif(image_file, save_path)

    return jsonify({'message': 'Prediction completed and saved as TIFF image', 'image_path': saved_image_path, 'result': result': result})
